  ...
  protected override updateMediaList(player: MediaPlayer): boolean {
    ...
    try {
      ...
      const promiseUpdateMediaList = new Promise((resolve, reject) => {
        ...
        this.spotifyPlusService.GetPlaylistFavorites(player.id, 0, 0, limitTotal, sortResult)
          .then(result => {
          ...
          .catch(error => {
            ...
            super.updatedMediaListError("Get Playlist Followed failed: \n" + (error as Error).message);
      ...
      super.updatedMediaListError("Playlist followed refresh failed: \n" + (error as Error).message);
      return true;
